...
A: Set a clear termination condition in your system prompt or set `maxTurns` in your `ToolExecutionConfig` to prevent infinite tool-calling loops. Example: `Agent(..., maxTurns = 10)`.
...
A: Ensure you have set the deployment name correctly in your environment. Example: `export AZURE_OPENAI_DEPLOYMENT=your-deployment-name` and your base URL matches `https://<resource>.openai.azure.com/`.
...
A: Verify you have set `LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, and `LANGFUSE_HOST`. Crucially, ensure you invoke `Tracing.flush()` before your application exits to avoid dropping pending trace batches.

**Q: Getting RateLimitError: 429 Too Many Requests frequently**  
A: Use `LLMClientRetry` or the `ReliabilityConfig` to automatically handle 429s with exponential backoff. Example: `ReliabilityConfig.default.copy(maxRetries = 3)`.
